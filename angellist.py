...
class AngelList(object):
    def __init__(self):
        """
        AngelList Base class that simply implements AngelList OAuth
        Authorization and
        AngelList APIs such as Activity Feeds, Follows, Reviews, Startups,
        Startup Roles, Status Updates, Tags, and Users.

        Please create an application from the link below if you do
        not have an API key and secret key yet.
    ...
    def getAuthorizeURL(self, client_id=None):
        ...
        if self.client_id is None:
            raise AngelListError("client_id is NULL. Please set this"
                                 "or pass it as a parameter first.")
        return "%s%s?client_id=%s&response_type=code" % (
          self.OAUTH_ENDPOINT, self.AUTHORIZATION_URL, self.client_id)
    ...
    def getAccessToken(self, client_id=None, client_secret=None, code=None):
        ...
        if self.client_secret is None:
            raise AngelListError("client_secret is NULL. Please "
                                 "set this or pass it as a parameter first.")

        if code is None:
            raise AngelListError(
              "code is NULL. Please pass the "
              "REQUEST['code'] angel.co/api/oauth/authorize "
              "responeded with as a parameter.")

        url = "%s%s?client_id=%s&client_secret=%s&code=%s&" % (
                self.OAUTH_ENDPOINT,
        ...
        try:
            ...
            # access token failed to fetch
            #(for any reason); so we'll just return blank
            access_token = ''
    ...
    def getFeed(self, access_token=None):
        ...
        return self.do_get_request(
          '%s/1/feed?access_token=%s' % (self.API_ENDPOINT, self.access_token))

    ##########################################################
    # Follows (http://angel.co/api/spec/follows)
    ### NOT WORKING YET [[DELETE ISSUE]]
    # (DELETE) https://api.angel.co/1/follows [type, id]
    def deleteFollows(self, access_token=None, follow_type=None,
                      follow_id=None):
        """
        ...
        if follow_type is None:
            raise AngelListError("the follow_type param is"
                                 "required for this api call.")

        if follow_id is None:
            raise AngelListError("the follow_id param is "
                                 "required for this api call.")

        data = {'type': follow_type, 'id': follow_id}
        url = "%s/1/follows?access_token=%s" % (
          self.API_ENDPOINT, self.access_token)
    ...
    def addFollows(self, access_token=None, follow_type=None, follow_id=None):
        ...
        if follow_id is None:
            raise AngelListError("the follow_id param is required "
                                 "for this api call.")
    ...
    def getFollowers(self, access_token=None, user_id=None):
        self.check_access_token(access_token)
        if user_id is None:
            raise AngelListError("the user_id param is required"
                                 "for this api call.")
        return self.do_get_request(
          '%s/1/users/%s/followers?access_token=%s' % (
            self.API_ENDPOINT, user_id, self.access_token))
    ...
    def getFollowersIds(self, access_token=None, user_id=None):
        ...
        if user_id is None:
            raise AngelListError("the user_id param is required "
                                 "for this api call.")
        return self.do_get_request(
          '%s/1/users/%s/followers/ids?access_token=%s' % (
            self.API_ENDPOINT, user_id, self.access_token))
    ...
    def getFollowing(self, access_token=None, user_id=None):
        ...
        if user_id is None:
            raise AngelListError("the user_id param is required"
                                 " for this api call.")
        return self.do_get_request(
          '%s/1/users/%s/following?access_token=%s' % (
            self.API_ENDPOINT, user_id, self.access_token))
    ...
    def getFollowingIds(self, access_token=None, user_id=None):
        ...
        if user_id is None:
            raise AngelListError("the user_id param is required "
                                 "for this api call.")
        return self.do_get_request(
          '%s/1/users/%s/following/ids?access_token=%s' % (
            self.API_ENDPOINT, user_id, self.access_token))
    ...
    def getStartupsFollowers(self, access_token=None, startup_id=None):
        ...
        if startup_id is None:
            raise AngelListError("the startup_id param is "
                                 "required for this api call.")
        return self.do_get_request(
          '%s/1/startups/%s/followers?access_token=%s' % (
            self.API_ENDPOINT, startup_id, self.access_token))
    ...
    def getStartupsFollowersIds(self, access_token=None, startup_id=None):
        ...
        if startup_id is None:
            raise AngelListError("the startup_id param is "
                                 "required for this api call.")
        return self.do_get_request(
          '%s/1/startups/%s/followers/ids?access_token=%s' % (
            self.API_ENDPOINT, startup_id, self.access_token))
    ...
    def getReviews(self, access_token=None, user_id=None):
        """
        user_id - OPTIONAL -
          id of the user you want reviews on (defaults to auth'ed user)
        """
        self.check_access_token(access_token)
        return self.do_get_request(
          '%s/1/reviews?access_token=%s&user_id=%s' % (
            self.API_ENDPOINT, self.access_token, user_id))
    ...
    def getStartups(self, access_token=None, startup_id=None):
        self.check_access_token(access_token)
        if startup_id is None:
            raise AngelListError("the startup_id param is required"
                                 "for this api call.")
        return self.do_get_request(
          '%s/1/startups/%s?access_token=%s' % (
            self.API_ENDPOINT, startup_id, self.access_token))
    ...
    def getStartupsSearch(self, access_token=None, slug=None, domain=None):
        ...
        """
        self.check_access_token(access_token)
        url = '%s/1/startups/search?access_token=%s' % (
          self.API_ENDPOINT, self.access_token)
        if slug:
    ...
    def getTagsStartups(self, access_token=None, tag_id=None):
        self.check_access_token(access_token)
        if tag_id is None:
            raise AngelListError("the tag_id param is required"
                                 " for this api call.")
        return self.do_get_request(
          '%s/1/tags/%s/startups?access_token=%s' % (
            self.API_ENDPOINT, tag_id, self.access_token))

    ##########################################################
    # Startup Roles (http://angel.co/api/spec/startup_roles)
    # (GET)    https://api.angel.co/1/startup_roles
    def getStartupRoles(self, access_token=None, user_id=None,
                        startup_id=None):
        """
        user_id - OPTIONAL -
           the user who's startup relationships you want to view
        startup_id - OPTIONAL -
            the startup who's user relationships you want to view
        """
        self.check_access_token(access_token)
        url = '%s/1/startups_roles?access_token=%s' % (
          self.API_ENDPOINT, self.access_token)
        if user_id:
    ...
    def getStatusUpdates(self, access_token=None, user_id=None,
                         startup_id=None):
        """
        user_id - OPTIONAL
        startup_id - OPTIONAL
        """
        self.check_access_token(access_token)
        url = '%s/1/startups_updates?access_token=%s' % (
          self.API_ENDPOINT, self.access_token)
        if user_id:
            url = '%s&user_id=%s' % (url, user_id)
        if startup_id:
    ...
    def postStatusUpdates(self, access_token=None,
                          startup_id=None, message=None):
        """
        ...
        self.check_access_token(access_token)
        if message is None:
            raise AngelListError("the message param is required"
                                 " for this api call.")
        data = {'message': message}
        if startup_id:
            data['startup_id'] = startup_id

        url = "%s/1/status_updates?access_token=%s" % (
          self.API_ENDPOINT, self.access_token)
        return self.do_post_request(url, data)
    ...
    def deleteStatusUpdates(self, access_token=None, status_id=None):
        ...
        if status_id is None:
            raise AngelListError("the status_id param is required"
                                 " for this api call.")
        data = {'id': status_id}
        url = "%s/1/status_updates?access_token=%s" % (
          self.API_ENDPOINT, self.access_token)
    ...
    def getTagsChildren(self, access_token=None, tag_id=None):
        ...
        if tag_id is None:
            raise AngelListError("the tag_id param is required"
                                 " for this api call.")
        return self.do_get_request(
          '%s/1/tags/%s/children?access_token=%s' % (
            self.API_ENDPOINT, tag_id, self.access_token))
    ...
    def getTagsParents(self, access_token=None, tag_id=None):
        ...
        if tag_id is None:
            raise AngelListError("the tag_id param is required"
                                 " for this api call.")
        return self.do_get_request(
          '%s/1/tags/%s/parents?access_token=%s' % (
            self.API_ENDPOINT, tag_id, self.access_token))
    ...
    def getMe(self, access_token=None):
        self.check_access_token(access_token)
        return self.do_get_request(
          '%s/1/me?access_token=%s' % (self.API_ENDPOINT, self.access_token))
